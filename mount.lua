...
local function reverse_animation(self, anim, output_name)
    if self.animation and self.animation[anim] then
        ...
        self.animation[output_name] = {
            range = {x = frame_x, y = frame_y},
            speed = -speed,
            loop = loop
        }
    end
...
minetest.register_on_leaveplayer(
    function(player) mob_core.force_detach(player) end)

minetest.register_on_shutdown(function()
    local players = minetest.get_connected_players()
    for i = 1, #players do mob_core.force_detach(players[i]) end
end)
...
function mob_core.attach(entity, player)
    ...
    local rot_view = 0
    if entity.player_rotation.y == 90 then rot_view = math.pi / 2 end
    ...
    minetest.after(0.2, function() animate_player(player, "sit", 30) end)
...
function mob_core.hq_mount_logic(self, prty)
    ...
    local func = function(self)
        ...
        elseif ctrl.down and self.isonground then -- move backwards
            if self.max_speed_reverse == 0 and vel == 0 then return end
        ...
        if self.isonground then
            if ctrl.jump then vel.y = (self.jump_height) + 4 end
        end
        -- stand
        if tvel ~= 0 and not ctrl.up or ctrl.down then tvel = tvel * 0.75 end
